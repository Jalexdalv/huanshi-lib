...
@Service
public class StunService extends AbstractService {
    @Autowired
    private Plugin plugin;
    private final Map<UUID, BossBar> bossBarMap = new ConcurrentHashMap<>();
    private final Timer timer = new Timer();

    public void run(@NotNull Player player, double time, boolean force) {
        UUID uuid = player.getUniqueId();
        timer.run(plugin, uuid, true, true, (int) Math.ceil(time * 2), 10, null,
            () -> {
                bossBarMap.putIfAbsent(uuid, BossBar.bossBar(Zh.stun(time), 1.0F, BossBar.Color.RED, BossBar.Overlay.NOTCHED_6));
                return true;
            }, restTime -> player.showBossBar(bossBarMap.get(uuid).name(Zh.stun(restTime)).progress((float) restTime / (float) time)),
            () -> {
