...
public class InvincibleService extends AbstractService {
    ...
    public void run(@NotNull Player player, double time) {
        ...
        timer.run(plugin, uuid, true, true, (int) Math.ceil(time * 2), 10, null,
            () -> {
                bossBarMap.putIfAbsent(uuid, BossBar.bossBar(Zh.invincible(time), 1.0F, BossBar.Color.WHITE, BossBar.Overlay.NOTCHED_6));
                return true;
            }, restTime -> player.showBossBar(bossBarMap.get(uuid).name(Zh.invincible(restTime)).progress((float) restTime / (float) time)),
            () -> {
