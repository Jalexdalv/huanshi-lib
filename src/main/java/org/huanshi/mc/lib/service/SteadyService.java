...
@Service
public class SteadyService extends AbstractService {
    @Autowired
    ...
    public void run(@NotNull Player player, long duration) {
        ...
        timer.run(plugin, uuid, true, true, duration, 0, period, null,
            () -> {
                bossBarMap.putIfAbsent(uuid, BossBar.bossBar(Component.empty(), 1.0F, BossBar.Color.GREEN, BossBar.Overlay.NOTCHED_6));
                return true;
            }, remainDuration -> player.showBossBar(bossBarMap.get(uuid).name(Zh.steady(remainDuration)).progress((float) remainDuration / (float) duration)),
            () -> {
