...
public class SteadyService extends AbstractService {
    ...
    public void run(@NotNull Player player, double time) {
        ...
        timer.run(plugin, uuid, true, true, (int) Math.ceil(time * 2), 10, null,
            () -> {
                bossBarMap.putIfAbsent(uuid, BossBar.bossBar(Zh.steady(time), 1.0F, BossBar.Color.GREEN, BossBar.Overlay.NOTCHED_6));
                return true;
            }, restTime -> player.showBossBar(bossBarMap.get(uuid).name(Zh.steady(restTime)).progress((float) restTime / (float) time)),
            () -> {
